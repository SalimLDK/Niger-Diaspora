...
fn alice_et_bob(conv: &str) -> (MlsEngine, MlsEngine) {
    ...
    let out = alice.add_members(conv, &[kp_bob]).unwrap();
...
fn message_d_un_epoch_passe_reste_lisible() {
    ...
    let out = alice.add_members(conv, &[kp]).unwrap();
    alice.merge_pending_commit(conv).unwrap();
    // OpenMLS remet l'AAD à vide après chaque MlsMessageOut : un commit
    // produit sans set_aad part donc avec un AAD vide. Le moteur de
    // production devra poser un AAD de commit explicite (§ 6.1 du plan).
    match bob.process_incoming(conv, &out.commit, &[]).unwrap() {
...
fn membre_retire_ne_lit_plus() {
    ...
    let leaf_bob = alice
        ...
        .unwrap()
        .leaf_index;
    let out = alice.remove_members(conv, &[leaf_bob]).unwrap();
    alice.merge_pending_commit(conv).unwrap();
    let a = aad(conv, "commit", "a1");
    // Bob traite le commit qui le retire : le groupe devient inactif pour lui.
    let _ = bob.process_incoming(conv, &out.commit, &[]);
...
fn commit_perdant_se_jette_proprement() {
    ...
    let out_alice = alice.add_members(conv, &[kp_c]).unwrap();
    let _out_bob = bob.add_members(conv, &[kp_d]).unwrap();
    ...
    match bob.process_incoming(conv, &out_alice.commit, &[]).unwrap() {
        Processed::Commit(snap) => assert_eq!(snap.epoch, 2),
        _ => panic!("attendu un commit"),
    }
