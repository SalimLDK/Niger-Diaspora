...
impl MlsEngine {
    ...
    pub fn add_members(&mut self, conversation_id: &str, key_packages: &[Vec<u8>]) -> Result<CommitOut, MlsError> {
    ...
    pub fn remove_members(&mut self, conversation_id: &str, leaf_indices: &[u32]) -> Result<CommitOut, MlsError> {
        let group = charger(&mut self.groups, self.provider.storage(), conversation_id)?;
