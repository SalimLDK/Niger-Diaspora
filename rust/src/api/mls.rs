...
impl Moteur {
    ...
    pub fn ajouter_membres(&mut self, conversation_id: String, key_packages: Vec<Vec<u8>>) -> anyhow::Result<CommitDto> {
        self.interne
    ...
    pub fn retirer_membres(&mut self, conversation_id: String, leaf_indices: Vec<u32>) -> anyhow::Result<CommitDto> {
        self.interne
            .remove_members(&conversation_id, &leaf_indices)
            .map(commit)
            .map_err(code)
    }
