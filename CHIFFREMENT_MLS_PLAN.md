...
| **8 — Groupes** — **jointure externe livrée le 2026-09-15** | la réconciliation gérait déjà N membres (le banc en joue trois) ; ce qui manquait était l'entrée dans un groupe ouvert **sans attendre personne** : l'arbre public (`GroupInfo`) est publié sur `conversations.mls_group_info` après chaque commit, et un arrivant s'ajoute lui-même par commit externe. Réservé aux conversations de type `group` — un 1:1 ne se rejoint pas tout seul. Migration `20260915180000` **à appliquer** ; d'ici là la colonne est lue à part et son absence ne coûte que la jointure externe | `select count(*) from mls_messages m join conversations c on c.id = m.conversation_id where c.type='group'` > 0 — le banc joue déjà un groupe ouvert que Charlie rejoint seul, puis lit et écrit. Reste : les chemins d'appartenance de l'app (invitations, exclusions, groupe officiel) et le groupe officiel en masse | 5–8 semaines |
