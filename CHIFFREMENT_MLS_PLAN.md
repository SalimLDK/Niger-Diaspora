...
| **5 — 1:1 derrière un flag** — **fondation posée le 2026-09-15**, branchement à venir | tables faites en phase 3 ; `MlsMessageMapper` (payload MLS → `MessageEntity`, placeholder pour un message illisible plutôt qu'un trou dans le fil, `content_type` grossier pour le serveur), `MlsSourceMerger` (fusion legacy + MLS, séparateur visible seulement quand les deux côtés existent), drapeau `featureFlags.mlsMessages` **fermé** et son provider | `select count(*) from mls_messages` > 0 entre deux comptes réels sur deux appareils réels — **pas encore** : la fusion n'est pas branchée dans le repository, et le drapeau est fermé. 27 tests Dart sur la fondation | 4–6 semaines |
