...
| **6 — Gel** — **PRÉPARÉE, non applicable le 2026-09-15** (`supabase/gel-messages-cible.sql`, hors `migrations/` pour que `db push` ne puisse pas l'appliquer ; banc `tools/mls_banc/gel_messages.sql`). **Le vrai verrou n'est pas celui qu'on croyait** (revu le 2026-09-15 au soir). Le verrou de version minimale EXISTE désormais (`version_minimale.dart`, secret `VERSION_MINIMALE_APP`), mais il porte un quatrième refus : il ne bloque jamais sur une version que le store n'offre pas encore. Or MLS est arrivé au versionCode **19**, et le Play Store en est au **18** — la publication a essuyé cinq refus. Tant qu'aucun build portant MLS n'est publié, poser `VERSION_MINIMALE_APP` ne fait rien, le délai ne peut pas commencer, et le gel reste hors de portée. **La phase 6 est bloquée par la publication, pas par le code.** Les deux autres conditions restent fausses de toute façon : le délai n'a pas commencé, et le trafic n'a pas basculé (13 messages chiffrés contre 122 en clair). Le gel n'emporte que `INSERT` : l'historique reste amendable (réactions, suppressions), ce que le banc mesure | délai de mise à jour minimale expiré (B) ; `REVOKE INSERT` ; triggers d'insertion retirés de `messages` ; `decrypt_aes_fallback` hors triggers ; annonce de purge (H) | un `insert` dans `messages` sous `SET LOCAL ROLE authenticated` échoue (sans `SET LOCAL ROLE`, `db query --linked` tourne en `postgres` et le test ment) | 1 semaine |
| **7 — Multi-appareil** — **code de sécurité FAIT le 2026-09-15**, le reste ouvert | `MlsCodeSecurite` : empreinte d'un appareil (identité MLS + clé publique de signature), code à 60 chiffres comparable de vive voix, charge QR et son analyseur, mémoire locale des vérifications avec **détection du changement de clé**. Affiché sous chaque ligne du registre MLS. **La décision E n'a plus à être prise globalement** (revu le 2026-09-15 au soir) : `featureFlags.multiAppareilComptes` ouvre le multi-session **par compte**, même motif que `mlsMessagesComptes`. On éprouve donc le multi-appareil sur un compte de test sans toucher à la posture de sécurité de personne. **Reste** : l'écran de scan, et le fait que la règle protège peut-être contre le partage de comptes, et que `KeyTransferService` en dépend (l'ancien téléphone dépose ses clés AVANT que le neuf ne se connecte, parce que se connecter l'éjecterait) | deux appareils d'un même compte affichent la même conversation ; un troisième révoqué ne déchiffre plus les suivants | 3–4 semaines |
