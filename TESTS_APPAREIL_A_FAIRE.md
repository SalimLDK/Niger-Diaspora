...
- [ ] **⚠ Le vivier de prékeys serveur est vide et ne se remplira pas tout seul** (`key_manager_service.dart:517-531`, constaté 2026-08-03) : `e2ee_one_time_prekeys` contient **0 ligne** en production. Le lancement de l'app sur l'appareil n'y a rien changé, et c'est logique : `checkAndRefillOneTimePreKeys` compare le seuil de 20 au compteur du **stockage local**, pas au serveur. Un appareil déjà pourvu localement ne republie donc jamais, même maintenant que RLS l'autorise. Conséquence à trancher : les installations existantes resteront sans prékey publiée, et seul un nouvel appareil (ou une réinstallation) en publiera. Reste à déterminer — je ne l'ai pas audité — si l'établissement de session X3DH sait se rabattre sur la signed pre-key seule (dégradation de la confidentialité persistante) ou s'il échoue franchement. Si c'est un échec, il faut une republication forcée pour le parc installé.
